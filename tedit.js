...
CookieManager = function(doc)
{
	this.document = doc;
}

CookieManager.prototype.setProperty = function(key, value)
{
	var date = new Date();
	date.setTime(date.getTime() + 1000 * 60 * 60 * 24 * 365);
	this.document.cookie = "%s=%s; %s=%s".format([key, value, "expires", date.toGMTString()]);
}

CookieManager.prototype.getProperty = function(key)
{
	var cookieStr = this.document.cookie;
	var cookies = cookieStr.split(";");
	for(var i = 0; i < cookies.length; i++)
	{
		var delimiter = cookies[i].indexOf("=");
		if(key.trim() == cookies[i].substring(0, delimiter).trim())
		{
			return cookies[i].substring(delimiter + 1);
		}
	}
	return "";
}

cm = new CookieManager(document);

tedit = function(domcontid, id)
{
	...
	this.textformat.formats["underline"] = {id: "u", style: "text-decoration: underline;"};
...
tedit.prototype.createEditor = function(domcont)
{
	...
	for(var i = 0; i < this.buttons.length; i++)
	{
		...
		{
			bt.innerHTML = button.value;
		}
		if((button.alt != "") && (button.alt != null))
		{
			bt.setAttribute("alt", button.alt);
		}
	...
	autopreview.checked = cm.getProperty("autoPreview");
	if(autopreview.checked)
	{
		this.autoPreviewUpdate = true;
	}
	...
	var savedText = cm.getProperty("text-" + this.id);
...
tedit.prototype.togglePreview = function(state)
{
	...
	cm.setProperty("autoPreview", (state ? "true" : ""));
...
{
	cm.setProperty("text-" + this.id, this.textarea.value);
	if(this.autoPreviewUpdate)
	{
		this.showPreview();
	}
...
tedit.prototype.showPreview = function()
{
	...
	for(var i = 0; i < formatParts.length; i++)
	{
		...
		console.log("Begin %s(%s); End %s(%s)".format([begin, part.begin, end, part.end]));
		var format = this.textformat.formats[part.type];
		var formatStart = this.textformat.formatStart.format(format.id);
		var formatEnd = this.textformat.formatEnd.format(format.id);
		var before = parsedStr.substring(0, begin);
		var after = parsedStr.substring(end + formatEnd.length);
		var text = parsedStr.substring(begin + formatStart.length, end);
		...
		var parts = html.split((html.indexOf(text) > -1 ? text : escapeHTML(text)));
		if(parts.length != 2) continue;
		parsedStr = before + html + after;
		console.log(parts);
		for(var j = part.begin; j < str.length; j++)
		{
			translation[j] += parts[0].length;
		}
		for(var k = part.end; k < str.length; k++)
		{
			translation[k] += parts[1].length - formatEnd.length - formatStart.length;
		}
		console.log(str);
